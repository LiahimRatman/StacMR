...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab, max_len, text_number, text_dim):
        ...
        # if data_split == 'dev':
            # FOR ORIGINAL FLICKR30k VAL OR CTC TEST:
            # self.length = 5000
            # FOR CTC EXPLICIT 738 VAL -> 738 * 5 = 3690
            # self.length = 3690
            # # FOR CTC FULL TEST -> 5000 * 5 = 25000
            # self.length = 25000
    ...
    def __getitem__(self, index):
        ...
        vocab = self.vocab
        # LOAD SCENE TEXT FEATURES
        text_features = self.text_features[img_id]
        text_features = text_features[:self.text_number][:]
        ...
        return image, target, index, img_id, caption_label, caption_mask, text_features
...
def collate_fn(data):
    ...
    images, captions, ids, img_ids, caption_labels, caption_masks, text_features = zip(*data)
